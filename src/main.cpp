        ...
        while (!window.shouldClose()) {
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (config.features.extra.enableBloomEffect) {
                    ...
                    uint32_t minScale = 1u, maxScale = 10u;
                    ImGui::SliderScalar("Scale", ImGuiDataType_U32, &config.features.scaleSize, &minScale, &maxScale);
                    
                    uint32_t minFilter = 1u, maxFilter = 13u;
                    ImGui::SliderScalar("Filter Size (pixels)", ImGuiDataType_U32, &config.features.filterSize, &minFilter, &maxFilter);
            ...
            if (viewMode == ViewMode::Rasterization) {
                ...
                if (debugBVHLeaf)
                    ImGui::SliderInt("BVH Leaf", &bvhDebugLeaf, 1, (int)bvh.numLeaves());
            }
